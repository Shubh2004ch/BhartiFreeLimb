import React, { useState, useEffect } from 'react';
import {
  Container,
  Typography,
  Grid,
  Card,
  CardContent,
  CardMedia,
  Box,
  Chip,
  Skeleton,
  Fade,
  Button,
  IconButton,
} from '@mui/material';
import {
  LocationOn,
  Phone as PhoneIcon,
  AccessTime,
  Email as EmailIcon,
  Hotel,
  Star,
  ArrowBack,
} from '@mui/icons-material';
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
import { ENDPOINTS, getImageUrl } from '../constants';
import api from '../services/api';

// Loading skeleton for beautiful loading state
const SleepingBagSkeleton = () => (
  <Grid container spacing={4}>
    {[1, 2, 3].map((i) => (
      <Grid item xs={12} md={6} lg={4} key={i}>
        <Card
          sx={{
            borderRadius: 4,
            boxShadow: 5,
            p: 2,
            minHeight: 350,
            bgcolor: 'background.paper',
          }}
        >
          <Skeleton variant="rounded" height={200} sx={{ borderRadius: 2, mb: 2 }} />
          <Skeleton variant="text" width="70%" height={36} />
          <Skeleton variant="text" width="60%" />
          <Skeleton variant="text" width="50%" />
          <Skeleton variant="rectangular" width="80%" height={30} sx={{ borderRadius: 2, mt: 2 }} />
        </Card>
      </Grid>
    ))}
  </Grid>
);

const SleepingBagsPage = () => {
  const [sleepingBags, setSleepingBags] = useState([]);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    const fetchSleepingBags = async () => {
      try {
        const response = await api.get(ENDPOINTS.SLEEPING_BAGS);
        setSleepingBags(response.data);
        setLoading(false);
      } catch (error) {
        console.error('Error fetching sleeping bags:', error);
        setLoading(false);
      }
    };
    fetchSleepingBags();
  }, []);

  return (
    <Box
      sx={{
        minHeight: '100vh',
        pt: 10,
        pb: 8,
        bgcolor: 'linear-gradient(120deg,#f0f9ff 0%,#fff7f7 100%)',
        background: 'linear-gradient(120deg,#e0ecff 0%,#fff6f6 100%)',
      }}
    >
      <Container maxWidth="lg">
        <Box display="flex" alignItems="center" mb={2}>
          <IconButton
            onClick={() => navigate(-1)}
            size="large"
            sx={{
              mr: 1,
              bgcolor: 'white',
              border: '1px solid #e0e7ef',
              boxShadow: 1,
              '&:hover': { bgcolor: '#f3f4f6' },
            }}
            aria-label="Back"
          >
            <ArrowBack sx={{ color: '#2563eb' }} />
          </IconButton>
          <Typography variant="h6" sx={{ color: '#2563eb', fontWeight: 700 }}>
            Back
          </Typography>
        </Box>
        <Box textAlign="center" mb={6}>
          <Hotel sx={{ fontSize: 46, color: '#2563eb', mb: 1 }} />
          <Typography
            variant="h3"
            sx={{
              fontWeight: 900,
              letterSpacing: 1.4,
              mb: 1,
              background: 'linear-gradient(90deg, #2563eb 30%, #f472b6 70%)',
              WebkitBackgroundClip: 'text',
              WebkitTextFillColor: 'transparent',
              display: 'inline-block',
            }}
          >
            Sleeping Bags
          </Typography>
          <Typography
            variant="h6"
            color="text.secondary"
            sx={{
              mb: 1,
              maxWidth: 600,
              mx: 'auto',
              fontWeight: 500,
            }}
          >
            Find free, safe, and comfortable sleeping bag facilities in your area. Rest with dignity and care.
          </Typography>
        </Box>

        {loading ? (
          <SleepingBagSkeleton />
        ) : (
          <Grid container spacing={5}>
            {sleepingBags.map((bag, idx) => (
              <Grid item xs={12} md={6} lg={4} key={bag._id}>
                <Fade in timeout={550} style={{ transitionDelay: `${idx * 100}ms` }}>
                  <Card
                    sx={{
                      borderRadius: 4,
                      boxShadow: '0 6px 22px 0 rgba(59,130,246,0.09), 0 2px 8px 0 rgba(236,72,153,0.09)',
                      height: '100%',
                      display: 'flex',
                      flexDirection: 'column',
                      overflow: 'hidden',
                      transition: 'transform 0.22s cubic-bezier(.4,0,.2,1), box-shadow 0.22s cubic-bezier(.4,0,.2,1)',
                      '&:hover': {
                        transform: 'translateY(-8px) scale(1.03)',
                        boxShadow:
                          '0 12px 36px 0 rgba(59,130,246,0.18), 0 3px 12px 0 rgba(236,72,153,0.16)',
                      },
                      bgcolor: 'rgba(255,255,255,0.92)',
                      backdropFilter: 'blur(3px)',
                      cursor: 'pointer'
                    }}
                    onClick={() => navigate(`/sleeping-bags/${bag._id}`)}
                  >
                    {bag.imagePath && (
                      <CardMedia
                        component="img"
                        height="200"
                        image={getImageUrl(bag.imagePath)}
                        alt={bag.name}
                        sx={{
                          objectFit: 'cover',
                          filter: 'brightness(0.98)',
                          borderTopLeftRadius: 16,
                          borderTopRightRadius: 16,
                        }}
                      />
                    )}
                    <CardContent
                      sx={{
                        display: 'flex',
                        flexDirection: 'column',
                        flexGrow: 1,
                        p: 3,
                      }}
                    >
                      <Typography
                        variant="h5"
                        sx={{
                          fontWeight: 'bold',
                          color: '#2563eb',
                          mb: 2,
                          letterSpacing: 0.5,
                        }}
                      >
                        {bag.name}
                      </Typography>

                      <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.2 }}>
                        <LocationOn sx={{ color: '#f472b6', mr: 1, fontSize: 20 }} />
                        <Typography variant="body1" sx={{ color: '#475569', fontWeight: 500 }}>
                          {bag.location}
                        </Typography>
                      </Box>

                      <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.2 }}>
                        <PhoneIcon sx={{ color: '#f472b6', mr: 1, fontSize: 20 }} />
                        <Typography variant="body1" sx={{ color: '#475569', fontWeight: 500 }}>
                          {bag.contactNumber}
                        </Typography>
                      </Box>

                      {bag.email && (
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                          <EmailIcon sx={{ color: 'info.main', fontSize: 18, mr: 1 }} />
                          <Typography variant="body2" sx={{ color: '#475569' }}>
                            {bag.email}
                          </Typography>
                        </Box>
                      )}
                      {bag.workingHours && (
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                          <AccessTime sx={{ color: '#fbbf24', fontSize: 19, mr: 1 }} />
                          <Typography variant="body2" sx={{ color: '#475569' }}>
                            {bag.workingHours}
                          </Typography>
                        </Box>
                      )}
                      {bag.rating && (
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                          <Star sx={{ color: '#fbbf24', fontSize: 19, mr: 1 }} />
                          <Typography variant="body2" sx={{ color: '#475569' }}>
                            {bag.rating}
                          </Typography>
                        </Box>
                      )}
                      {bag.tags && Array.isArray(bag.tags) && (
                        <Box sx={{ mt: 1.2, mb: 1, display: 'flex', flexWrap: 'wrap', gap: 0.7 }}>
                          {bag.tags.map((tag) => (
                            <Chip
                              key={tag}
                              label={tag}
                              size="small"
                              sx={{
                                bgcolor: 'primary.light',
                                color: 'primary.contrastText',
                                fontWeight: 600,
                                fontSize: 13,
                                letterSpacing: 0.3,
                              }}
                            />
                          ))}
                        </Box>
                      )}
                      {bag.description && (
                        <Typography
                          variant="body2"
                          sx={{
                            color: 'text.secondary',
                            mt: 2,
                            mb: 1,
                            fontSize: 15.5,
                            letterSpacing: 0.3,
                          }}
                        >
                          {bag.description}
                        </Typography>
                      )}
                      {bag.website && (
                        <Button
                          href={bag.website}
                          target="_blank"
                          size="small"
                          variant="outlined"
                          sx={{
                            mt: 'auto',
                            alignSelf: 'flex-start',
                            color: '#2563eb',
                            borderColor: '#2563eb',
                            textTransform: 'none',
                            fontWeight: 600,
                            letterSpacing: 0.2,
                            px: 2,
                            py: 0.5,
                            borderRadius: 99,
                            '&:hover': {
                              bgcolor: '#2563eb10',
                              borderColor: '#2563eb',
                            },
                          }}
                        >
                          Visit Website
                        </Button>
                      )}
                    </CardContent>
                  </Card>
                </Fade>
              </Grid>
            ))}
          </Grid>
        )}
      </Container>
    </Box>
  );
};

export default SleepingBagsPage;